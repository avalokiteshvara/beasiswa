...
class Admin_model extends CI_Model
{
    ...
    function pendaftar($kategori_id){
      ...
      if($kategori['level_penerima'] === 'pelajar'){
        //query pelajar
        $this->db->select(
          "a.nama_lengkap,a.kab_kota,a.jk,a.nama_lembaga,a.kelas, a.email,a.id,a.nik, a.alamat_rumah,
        ...
        $this->db->select(
          "a.email,a.id,a.nik,a.nama_lengkap, a.alamat_rumah,a.kab_kota, a.jk, a.nama_lembaga, a.program_studi,a.akreditasi, a.semester,
		       a.ip_semester, a.`status`, GROUP_CONCAT(CONCAT(c.id,'|', REPLACE(b.file_dokumen,a.nik,''),'|',b.verifikasi) SEPARATOR ';') AS dokumen"
        );
        $this->db->join("dokumen_pendaftar b","a.id = b.pendaftar_id","left");
        $this->db->join("jenis_dokumen c","b.jenis_dokumen_id = c.id","left");
        $this->db->group_by('a.id');
