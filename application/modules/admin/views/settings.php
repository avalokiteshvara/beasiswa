      ...
      <table width="100%" class="table table-condensed">
        ...
        <tbody>
          ...
          <?php foreach ($setting->result() as $r) { ?>
            <tr>
              <td><?php echo ucwords( str_replace('_',' ', $r->title) ); ?></td>
              <td>
                <?php if ($r->tipe === 'big-text') { ?>
                  <textarea id="<?php echo $r->id; ?>" class="texteditor" style="width: 100%;height: 100%"><?php echo $r->value; ?></textarea>
                  <script>
                ...
                <?php } elseif ($r->tipe === 'small-text') { ?>
                  <input id="<?php echo $r->title; ?>" class="form-control" style="width: 100%;height: 100%" type="text" value="<?php echo $r->value; ?>">
                  <script>
                    $('#<?php echo $r->title; ?>').keyup(function() {
                ...
                <?php } elseif ($r->tipe === 'link') { ?>
                  <a href="<?php echo base_url($r->value); ?>/0"> Manage</a>
                <?php } elseif ($r->tipe === 'option') {
                  $option_setting = explode(';', $r->value ?? '');
                  $option_list = explode(',', $option_setting[0]);

                  if (count($option_setting) > 1) {
                    $option_selected = $option_setting[1];

                    echo '<select id="title_' . $r->title . '" name="' . $r->title . '" class="form-control">';
                  ...
                  <script>
                    $('#title_<?php echo $r->title; ?>').on('change', function() {
                      var this_title = $(this).attr('name');
                      var options = $('#title_<?php echo $r->title; ?> option');
